...
def min_schedule(domain,read,schedule,C = 100):
    ...
    deps = read.apply_range(read.reverse()).apply_domain(schedule).apply_range(schedule).intersect(isl.Map("{[i]->[j] : i<j}"))
...
def json_file_to_isl(file_path: str,with_single: bool = True,reschedule: str = "default"): 
    ...
    if not with_single:
        multi_keys = { key for key, vals in access_read.items() if len(vals) > 1 }
        access_read = { key: vals for key, vals in access_read.items() if key in multi_keys }
        access_write = { key: vals for key, vals in access_write.items() if key in multi_keys }
